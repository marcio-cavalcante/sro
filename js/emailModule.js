// Função para extrair repasse e contrapartida de forma inteligente
function extrairRepasseEContrapartida(texto) {
    const linhas = texto.split('\n');
    let repasse = "";
    let contrapartida = "";
    
    // Procurar a linha que contém repasse
    for (const linha of linhas) {
        if (linha.includes("Repasse:")) {
            if (linha.includes("Contrapartida:") || linha.includes(" - ")) {
                // Formato: "Repasse: X - Contrapartida: Y"
                const partes = linha.split("-");
                if (partes.length >= 2) {
                    repasse = partes[0].split("Repasse:")[1].trim();
                    
                    // Verificar se a segunda parte contém "Contrapartida:" explicitamente
                    if (partes[1].includes("Contrapartida:")) {
                        contrapartida = partes[1].split("Contrapartida:")[1].trim();
                    } else {
                        // Se não contém "Contrapartida:" explicitamente, usar toda a segunda parte
                        contrapartida = partes[1].trim();
                    }
                }
            } else {
                // Formato: "Repasse: X" (sem contrapartida na mesma linha)
                repasse = linha.substring(linha.indexOf("Repasse:") + "Repasse:".length).trim();
            }
            break;
        }
    }
    
    // Se não encontrou a contrapartida na linha do repasse, procurar em uma linha separada
    if (!contrapartida) {
        for (const linha of linhas) {
            if (linha.includes("Contrapartida:") && !linha.includes("Repasse:")) {
                contrapartida = linha.substring(linha.indexOf("Contrapartida:") + "Contrapartida:".length).trim();
                break;
            }
        }
    }
    
    return { repasse, contrapartida };
...
export function enviarEmailPendencias() {
    try {
        ...
        // Extrair valores e ajustar o formato
        let valorMedicao = extrairValor(textoApontamento, "Recebemos a Medição no valor de", ",");
        
        // Extrair valores de repasse e contrapartida usando a nova função
        const { repasse, contrapartida } = extrairRepasseEContrapartida(textoApontamento);
        
        // Extrair ordem do desbloqueio e parcela usando a função existente
    ...
    <div style="margin: 15px 0;">
        <table style="width: 20%; table-layout: fixed; border-collapse: collapse; border: 1px solid #000;">
            <tr>
                ...
                <td style="padding: 5px 10px; width: 40%; border: 1px solid #000; text-align: right; white-space: nowrap;">R$ ${valorMedicao}</td>
            </tr>
            <tr>
                <td style="padding: 5px 10px; border: 1px solid #000; white-space: nowrap;">Repasse:</td>
                <td style="padding: 5px 10px; border: 1px solid #000; text-align: right; white-space: nowrap;">R$ ${repasse}</td>
            </tr>
            <tr>
                <td style="padding: 5px 10px; border: 1px solid #000; white-space: nowrap;">Contrapartida:</td>
                <td style="padding: 5px 10px; border: 1px solid #000; text-align: right; white-space: nowrap;">R$ ${contrapartida}</td>
            </tr>
        </table>
    </div>
    ...
    <div style="margin: 15px 0;">
        <table style="width: 80%;">
            <tr>
                ...
                <td style="padding: 5px 10px; text-align: left;"><strong>${empresa 
                  ? (valorEmpresa 
            ...
            <tr>
                ...
                <td style="padding: 5px 10px; text-align: left;"><strong>${ordemDesbloqueio} - Parcela nº: ${parcela}</strong></td>
            </tr>
            <tr>
                <td style="padding: 5px 10px; vertical-align: top;">Instrum. desbloq.:</td>
                <td style="padding: 5px 10px; text-align: left;"><strong>${instrumento} com evolução de ${evolucao}</strong></td>
            </tr>
        </table>
    </div>`;
...
function extrairValorApos(texto, marcador) {
    ...
    for (const linha of linhas) {
        if (linha.includes(marcador)) {
            // Se for o repasse, extrai apenas até o hífen (se houver)
            if (marcador === "Repasse:" && linha.includes("-")) {
                return linha.substring(
                    linha.indexOf(marcador) + marcador.length, 
                    linha.indexOf("-")
                ).trim();
            }
            
            // Para outros casos, extrair o restante da linha
